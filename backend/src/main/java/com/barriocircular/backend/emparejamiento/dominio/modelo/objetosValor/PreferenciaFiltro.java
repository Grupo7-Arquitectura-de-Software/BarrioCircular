...
public record PreferenciaFiltro(Set<TipoMaterialFiltro> tiposMaterial, double radioMaximoKm, String zonaDescriptiva) {
    ...
    public PreferenciaFiltro {
        ...
        if (tiposMaterial.contains(null)) {
        ...
        if (!Double.isFinite(radioMaximoKm) || radioMaximoKm <= 0 || radioMaximoKm > RADIO_MAXIMO_KM) {
            throw new FiltroInvalidoException("El radio maximo (km) debe ser mayor que 0 y menor o igual a 50. Recibido:", radioMaximoKm);
        }
