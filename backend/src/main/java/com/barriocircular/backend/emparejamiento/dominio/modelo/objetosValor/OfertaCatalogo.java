...
public record OfertaCatalogo(UUID publicacionId, TipoMaterialFiltro tipoResiduo, double pesoKg,
                             BigDecimal precioPorKilo, double latitud, double longitud, String estado) {
    public OfertaCatalogo {
        ...
        if (precioPorKilo == null || precioPorKilo.signum() <= 0) {
            throw new OfertaCatalogoInvalidaException(
                    "El precioPorKilo de la oferta de catalogo debe ser mayor que 0. Recibido: " + precioPorKilo
            );
        }
        if (estado == null || estado.isBlank()) {
            throw new OfertaCatalogoInvalidaException("La oferta de catalogo requiere un estado no vacio.");
        }
