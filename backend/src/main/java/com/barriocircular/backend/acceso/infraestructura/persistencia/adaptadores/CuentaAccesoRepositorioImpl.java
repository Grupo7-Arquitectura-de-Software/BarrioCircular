...
public class CuentaAccesoRepositorioImpl implements CuentaAccesoRepositorio {
    ...
    @Override
    public Optional<CuentaAcceso> buscarPorId(UUID identificadorCuenta) {
        return datacuentarepositorio.findByIdentificadorCuenta(identificadorCuenta)
                .map(cuentaAccesoMapper::alDominio);
    }

    @Override
    public Optional<CuentaAcceso> buscarPorClerkId(String clerkId) {
        return datacuentarepositorio.findByIdentificadorUsuarioClerk(clerkId).map(cuentaAccesoMapper::alDominio);
    }
