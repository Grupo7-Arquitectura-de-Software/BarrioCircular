...
public record InformacionContacto(String correoElectronico, String telefono) {
    ...
    private static final Pattern FORMATO_TELEFONO = Pattern.compile(
            "^(?:09\\d{8}|0[2-7]\\d{7}|\\+593(?:9\\d{8}|[2-7]\\d{7}))$");

    public InformacionContacto {
        if (correoElectronico == null || !FORMATO_CORREO.matcher(correoElectronico).matches()) {
            throw new InformacionContactoInvalidaException("El correo electronico del perfil no tiene un formato valido");
        }
