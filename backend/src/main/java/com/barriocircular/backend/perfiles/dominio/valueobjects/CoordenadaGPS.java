...
public record CoordenadaGPS(double latitud, double longitud) {
    ...
    public CoordenadaGPS {
        if (!Double.isFinite(latitud) || !Double.isFinite(longitud)
            ...
            throw new UbicacionFueraDeQuitoException("La coordenada GPS no se encuentra dentro de los rangos globales validos");
        }
        if (latitud < LATITUD_MINIMA_QUITO || latitud > LATITUD_MAXIMA_QUITO
                || longitud < LONGITUD_MINIMA_QUITO || longitud > LONGITUD_MAXIMA_QUITO) {
