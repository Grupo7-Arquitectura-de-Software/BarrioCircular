...
public class ActualizarDatosPerfilUseCase {
    ...
    public PerfilResultado ejecutar(ActualizarDatosPerfilCommand command) {
        PerfilUsuario perfil = perfilUsuarioRepository.buscarPorId(command.perfilId())
                .orElseThrow(() -> new PerfilDomainException("No existe un perfil con id " + command.perfilId()));

        if (command.correoElectronico() != null || command.telefono() != null) {
            String correo = command.correoElectronico() != null
                    ? command.correoElectronico()
                    : perfil.getInformacionContacto().getCorreoElectronico();
            String telefono = command.telefono() != null
                    ? command.telefono()
        ...
        if (command.latitud() != null || command.longitud() != null) {
            double latitud = command.latitud() != null ? command.latitud() : perfil.getUbicacionHabitual().getLatitud();
            double longitud = command.longitud() != null ? command.longitud() : perfil.getUbicacionHabitual().getLongitud();
